%{
This script is for analyzing from 700 to 1000 ms after the optically
stimulation pulse (3 ms). This is the time period when the events will no longer be
...
%{
I am interested in events that are between 700 to 1000 ms following the
...
Detection(1).start = OPTO.time_ms + 700;
Detection(1).stop = OPTO.time_ms + 1000;
...
writetable(Strontium(1).frequency_table,'Strontium_analyzed_700_1000.xlsx','Sheet','frequency','Range','A1');
writetable(Strontium(1).mean_amplitude_table,'Strontium_analyzed_700_1000.xlsx','Sheet','amplitude','Range','A1');
writetable(Strontium(1).AverageTrace_peak_table,'Strontium_analyzed_700_1000.xlsx','Sheet','averagetrace_peak','Range','A1');

% Table for exclusions (30pA)
writetable(Strontium(1).excluded30(1).frequency_table,'Strontium_analyzed_700_1000.xlsx','Sheet','frequency_exclusions30','Range','A1');
writetable(Strontium(1).excluded30(1).mean_amplitude_table,'Strontium_analyzed_700_1000.xlsx','Sheet','amplitude_exclusions30','Range','A1');
writetable(Strontium(1).excluded30(1).AverageTrace_peak_table,'Strontium_analyzed_700_1000.xlsx','Sheet','averagetrace_peak_exclusions30','Range','A1');