function [Group] = plotephys_Strontium_average(group);
    ...
    % averaged trace.
    display([newline 'select Strontium data for ' group]);
    ...
    for n = 1:length(filename)-2
        ...
        sweepLength = 3;    % each sweep is 3 seconds long
        ...
        %%
        optoStim = 1.0468; % first opto stim occurs at 1.07825
        ...
        beforeStim = optoStim + 0.0012; % Add 1.2 ms for when I should start looking for the evoked current
        afterStim = beforeStim + 0.0120; % Add 12 ms for when the peak should have ended
        ...
        Group(n).X = t_vec';
        Group(n).peakAmplitude = realPeaks;
        Group(n).peakTime = realPeakTimes;
        Group(n).optostimTime = optoStim_time;

        %{
        figure; hold on;
        plot(Group(n).X, Group(n).trace);
        plot(Group(n).peakTime, Group(n).peakAmplitude, 'g*');
        plot(Group(n).optostimTime, [0 0 0 0 0], 'rv');
        ylabel('Amplitude (pA)'); xlabel('time (s)');
        title(['5Hz' {Group.group} {Group(n).name}]);
        hold off;
        %}
    end
end
