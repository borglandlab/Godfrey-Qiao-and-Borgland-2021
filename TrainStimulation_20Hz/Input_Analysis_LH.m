...
% Imported a table with my cells that were non-responders and named it
% LH.NonRespondres

% Male Control
MC = LH.NonResponders;
MC_index = MC.Sex == "Male" & MC.Group == "Control";
MC = MC(MC_index, :);
LH.MaleControl.TRAIN.twentyHz(1).NonResponders = MC;
clear MC MC_index

% Male Fasted
MF = LH.NonResponders;
MF_index = MF.Sex == "Male" & MF.Group == "Fasted";
MF = MF(MF_index, :);
LH.MaleFasted.TRAIN.twentyHz(1).NonResponders = MF;
clear MF MF_index

% Female Control
FC = LH.NonResponders;
FC_index = FC.Sex == "Female" & FC.Group == "Control";
FC = FC(FC_index, :);
LH.FemaleControl.TRAIN.twentyHz(1).NonResponders = FC;
clear FC FC_index

% Female Fasted
FF = LH.NonResponders;
FF_index = FF.Sex == "Female" & FF.Group == "Fasted";
FF = FF(FF_index, :);
LH.FemaleFasted.TRAIN.twentyHz(1).NonResponders = FF;
clear FF FF_index

%% Double check that I'm not repeating anything

% Male Control
number = 0;
for n = 1 : height(LH.MaleControl.TRAIN.twentyHz(1).NonResponders)
    for m = 1 : length(LH.MaleControl.TRAIN.twentyHz)
        if LH.MaleControl.TRAIN.twentyHz(1).NonResponders.Acq(n) == LH.MaleControl.TRAIN.twentyHz(m).acq(:,1:end-4)
            display('the cell ' + LH.MaleControl.TRAIN.twentyHz(1).NonResponders.Acq(n) + ' is a repeat for Male Control')
            number = number + 1;
        end
    end
    if number < 1
        display('no repeats for Male Control cell ' + LH.MaleControl.TRAIN.twentyHz(1).NonResponders.Acq(n))
    end
end

% Male Fasted
number = 0;
for n = 1 : height(LH.MaleFasted.TRAIN.twentyHz(1).NonResponders)
    for m = 1 : length(LH.MaleFasted.TRAIN.twentyHz)
        if LH.MaleFasted.TRAIN.twentyHz(1).NonResponders.Acq(n) == LH.MaleFasted.TRAIN.twentyHz(m).acq(:,1:end-4)
            display('the cell ' + LH.MaleFasted.TRAIN.twentyHz(1).NonResponders.Acq(n) + ' is a repeat for Male Fasted')
            number = number + 1;
        end
    end
    if number < 1
        display('no repeats for Male Fasted cell ' + LH.MaleFasted.TRAIN.twentyHz(1).NonResponders.Acq(n))
    end
end

% Female Control
number = 0;
for n = 1 : height(LH.FemaleControl.TRAIN.twentyHz(1).NonResponders)
    for m = 1 : length(LH.FemaleControl.TRAIN.twentyHz)
        if LH.FemaleControl.TRAIN.twentyHz(1).NonResponders.Acq(n) == LH.FemaleControl.TRAIN.twentyHz(m).acq(:,1:end-4)
            display('the cell ' + LH.FemaleControl.TRAIN.twentyHz(1).NonResponders.Acq(n) + ' is a repeat for Female Control')
            number = number + 1;
        end
    end
    if number < 1
        display('no repeats for Female Control cell ' + LH.FemaleControl.TRAIN.twentyHz(1).NonResponders.Acq(n))
    end
end

% Female Fasted
number = 0;
for n = 1 : height(LH.FemaleFasted.TRAIN.twentyHz(1).NonResponders)
    for m = 1 : length(LH.FemaleFasted.TRAIN.twentyHz)
        if LH.FemaleFasted.TRAIN.twentyHz(1).NonResponders.Acq(n) == LH.FemaleFasted.TRAIN.twentyHz(m).acq(:,1:end-4)
            display('the cell ' + LH.FemaleFasted.TRAIN.twentyHz(1).NonResponders.Acq(n) + ' is a repeat for Female Fasted')
            number = number + 1;
        end
    end
    if number < 1
        display('no repeats for Female Fasted cell ' + LH.FemaleFasted.TRAIN.twentyHz(1).NonResponders.Acq(n))
    end
end

clear m n number

%% Finding proportion of cells that didn't have a response (or response less than 50 pA)

% Male Control
% Actual Number
LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.number = length(LH.MaleControl.greater50(LH.MaleControl.greater50 == 1));
LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number = length(LH.MaleControl.greater50(LH.MaleControl.greater50 == 0));
LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.number = height(LH.MaleControl.TRAIN.twentyHz(1).NonResponders);

LH.MaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.number = LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.number;

MC_Sum = LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.number + LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.number;
% Fraction
LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.fraction = LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.number / MC_Sum;
LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.fraction = LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number / MC_Sum;
LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.fraction = LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.number / MC_Sum;
LH.MaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction = LH.MaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.number / MC_Sum;

clear MC_Sum

% Male Fasted
% Actual Number
LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.number = length(LH.MaleFasted.greater50(LH.MaleFasted.greater50 == 1));
LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number = length(LH.MaleFasted.greater50(LH.MaleFasted.greater50 == 0));
LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number = height(LH.MaleFasted.TRAIN.twentyHz(1).NonResponders);

LH.MaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.number = LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number;

MF_Sum = LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.number + LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number;
% Fraction
LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.fraction = LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.number / MF_Sum;
LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.fraction = LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number / MF_Sum;
LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.fraction = LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number / MF_Sum;
LH.MaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction = LH.MaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.number / MF_Sum;

clear MF_Sum

% Female Control
% Actual Number
LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.number = length(LH.FemaleControl.greater50(LH.FemaleControl.greater50 == 1));
LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number = length(LH.FemaleControl.greater50(LH.FemaleControl.greater50 == 0));
LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.number = height(LH.FemaleControl.TRAIN.twentyHz(1).NonResponders);

LH.FemaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.number = LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.number;

FC_Sum = LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.number + LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.number;MC
% Fraction
LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.fraction = LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.number / FC_Sum;
LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.fraction = LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number / FC_Sum;
LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.fraction = LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.number / FC_Sum;
LH.FemaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction = LH.FemaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.number / FC_Sum;

clear FC_Sum

% Female Fasted
% Actual Number
LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.number = length(LH.FemaleFasted.greater50(LH.FemaleFasted.greater50 == 1));
LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number = length(LH.FemaleFasted.greater50(LH.FemaleFasted.greater50 == 0));
LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number = height(LH.FemaleFasted.TRAIN.twentyHz(1).NonResponders);

LH.FemaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.number = LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number;

FF_Sum = LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.number + LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number;
% Fraction
LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.fraction = LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.number / FF_Sum;
LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.fraction = LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number / FF_Sum;
LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.fraction = LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number / FF_Sum;
LH.FemaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction = LH.FemaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.number / FF_Sum;

clear FF_Sum

%% Create a table showing the fractions

% Male Control
MC_Group(1,1) = {'Male Control'};
MC_Group(1,2) = {LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.number + LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.number;};
MC_Group(1,3) = {LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.number};
MC_Group(1,4) = {LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number};
MC_Group(1,5) = {LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.number};
MC_Group(1,6) = {LH.MaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.number};
MC_Group(1,7) = {LH.MaleControl.TRAIN.twentyHz(1).Group.Responder.fraction};
MC_Group(1,8) = {LH.MaleControl.TRAIN.twentyHz(1).Group.WeakResponder.fraction};
MC_Group(1,9) = {LH.MaleControl.TRAIN.twentyHz(1).Group.NonResponder.fraction};
MC_Group(1,10) = {LH.MaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction};

% Male Fasted
MF_Group(1,1) = {'Male Fasted'};
MF_Group(1,2) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.number + LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number;};
MF_Group(1,3) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.number};
MF_Group(1,4) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number};
MF_Group(1,5) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number};
MF_Group(1,6) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.number};
MF_Group(1,7) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.Responder.fraction};
MF_Group(1,8) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.fraction};
MF_Group(1,9) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.NonResponder.fraction};
MF_Group(1,10) = {LH.MaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction};

% Female Control
FC_Group(1,1) = {'Female Control'};
FC_Group(1,2) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.number + LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.number;};
FC_Group(1,3) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.number};
FC_Group(1,4) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.number};
FC_Group(1,5) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.number};
FC_Group(1,6) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.number};
FC_Group(1,7) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.Responder.fraction};
FC_Group(1,8) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.WeakResponder.fraction};
FC_Group(1,9) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.NonResponder.fraction};
FC_Group(1,10) = {LH.FemaleControl.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction};

% Female Fasted
FF_Group(1,1) = {'Female Fasted'};
FF_Group(1,2) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.number + LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number + LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number;};
FF_Group(1,3) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.number};
FF_Group(1,4) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.number};
FF_Group(1,5) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.number};
FF_Group(1,6) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.number};
FF_Group(1,7) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.Responder.fraction};
FF_Group(1,8) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.WeakResponder.fraction};
FF_Group(1,9) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.NonResponder.fraction};
FF_Group(1,10) = {LH.FemaleFasted.TRAIN.twentyHz(1).Group.Weak_NonResponder.fraction};

Group_array = [MC_Group; MF_Group; FC_Group; FF_Group];

LH.tables.GroupResponses = array2table(Group_array, 'VariableNames', {'Group', 'total_number', 'responder_number', 'weakresponder_number', 'nonresponder_number', 'weak_nonresponder_number', 'responder_fraction', 'weakresponder_fraction', 'nonresponder_fraction', 'weak_nonresponder_fraction'});

clear MC_Group MF_Group FC_Group FF_Group Group_array
...
writetable(LH.tables.GroupResponses,'LHinput_responses.xlsx','Sheet','responses','Range','A1');
