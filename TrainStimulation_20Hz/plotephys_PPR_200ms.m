function [Group] = plotephys_PPR_200ms(groupName)
...
% averaged trace.
display([newline 'select 200ms data for ' groupName]);
...
for n = 1:length(filename)
    if contains(filename(n).name,'abf') % I have to do this so that I'm not picking up .DS_Store files or anything other than .abf files
        ...
        ISI = 0.200;     % interstimulus interval is 25 ms
...
title(['PPR 200ms interval' {Group.group} {Group(o).name}]);
hold off;
        %}
    end
end
