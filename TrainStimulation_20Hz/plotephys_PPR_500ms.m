function [Group] = plotephys_PPR_500ms(groupName)
...
% averaged trace.
display([newline 'select 500ms data for ' groupName]);
...
for n = 1:length(filename)
    if contains(filename(n).name,'abf') % I have to do this so that I'm not picking up .DS_Store files or anything other than .abf files
        ...
        ISI = 0.500;     % interstimulus interval is 25 ms
...
title(['PPR 500ms interval' {Group.group} {Group(o).name}]);
hold off;
        %}
    end
end
