function [Group] = plotephys_PPR_5s(groupName)
...
% averaged trace.
display([newline 'select 5s data for ' groupName]);
...
for n = 1:length(filename)
    if contains(filename(n).name,'abf') % I have to do this so that I'm not picking up .DS_Store files or anything other than .abf files
        ...
        sweepLength = 10;    % each sweep is 4 seconds long
...
title(['PPR 5s interval' {Group.group} {Group(o).name}]);
hold off;
        %}
    end
end
