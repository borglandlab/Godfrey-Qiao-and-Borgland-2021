function [Group] = plotephys_PPR_1s(groupName)
...
% averaged trace.
display([newline 'select 1s data for ' groupName]);
...
for n = 1:length(filename)
    if contains(filename(n).name,'abf') % I have to do this so that I'm not picking up .DS_Store files or anything other than .abf files
        ...
        sweepLength = 6;    % each sweep is 4 seconds long
...
title(['PPR 1s interval' {Group.group} {Group(o).name}]);
hold off;
        %}
    end
end
